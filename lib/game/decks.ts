...
export function validateDeck(deck: DeckDefinition): string[] {
  const errors: string[] = [];
  if (deck.monsterIds.length !== 3 || new Set(deck.monsterIds).size !== 3)
  ...
  for (const id of counts.keys()) {
    ...
    if (owner && owner !== 'Any' && owner !== 'Breeder' && !owners.has(owner))
      errors.push(`${id} (${owner}) is incompatible with this team.`);
  }
