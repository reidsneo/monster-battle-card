...
import { STARTER_DECKS, validateDeck } from './decks';
import { createGame, getLegalActions, getLegalDefenses, observeGame, reduceGame } from './engine';
import type { CardInstance, GameState, PlayerIndex } from './types';

const instance = (cardId: string, tag = 'test'): CardInstance => ({ cardId, instanceId: `${tag}-${cardId}` });

function autoStep(state: GameState) {
  if (state.phase === 'setup-guts') return reduceGame(state, { type: 'finish-setup' });
  const target = state.pendingAttack?.targets[state.pendingAttack.targetCursor]?.player;
  const perspective = (state.phase === 'defense' ? target : state.activePlayer) as PlayerIndex;
  const request = {
    observation: observeGame(state, perspective),
    actions: getLegalActions(state), defenses: getLegalDefenses(state), difficulty: state.difficulty,
    entropy: state.seed ^ state.turn ^ state.eventSequence,
  } as const;
  return reduceGame(state, chooseAiCommand(request));
}

function autoMatch(player: 'miracle' | 'speed' | 'powerful', rival: 'miracle' | 'speed' | 'powerful', seed: number) {
  let state = createGame(player, 'hard', rival, seed);
  let steps = 0;
  while (state.phase !== 'gameover' && steps < 1500) { state = autoStep(state); steps += 1; }
  return { state, steps };
}
...
describe('verified content', () => {
  ...
  it('authors every starter card as a structured implemented definition', () => {
    ...
    expect(STARTER_CARDS.every((card) => card.implemented && card.image.endsWith(`${card.id}.webp`))).toBe(true);
    expect(MONSTERS).toHaveLength(9);
    const kinds = new Set(STARTER_CARDS.flatMap((card) => card.effects.map((effect) => effect.kind)));
    expect(kinds).toEqual(new Set(['combo', 'undodgeable', 'aoe', 'self-damage', 'guts-loss', 'half-on-dodge', 'lifesteal', 'dodge', 'block', 'reflect', 'redirect', 'jump', 'draw', 'discard-opponent', 'heal', 'double-if-low-life', 'lock-dodge', 'prevent-ko', 'repeatable']));
  });
  ...
  it.each(STARTER_DECKS)('$name is an exact legal starter fixture', (deck) => {
    ...
    expect(Math.max(...Object.values(deck.skillIds.reduce<Record<string, number>>((counts, id) => ({ ...counts, [id]: (counts[id] ?? 0) + 1 }), {})))).toBeLessThanOrEqual(3);
  });
});
...
describe('deterministic engine', () => {
  ...
  it('offers the second player up to two opening Guts', () => {
    ...
    state = reduceGame(state, { type: 'setup-toggle-guts', instanceId: state.players[0].hand[0].instanceId });
    state = reduceGame(state, { type: 'setup-toggle-guts', instanceId: state.players[0].hand[1].instanceId });
  ...
  it('sends the top deck card to Guts when a turn begins with five cards', () => {
    ...
    state = reduceGame(state, { type: 'finish-attacking' });
    while (state.players[1].hand.length) state = reduceGame(state, { type: 'convert-guts', instanceId: state.players[1].hand[0].instanceId });
  ...
  it('resolves an attack and prevents the same monster attacking twice', () => {
    ...
    state.players[0].guts = [instance('112', 'g1'), instance('113', 'g2'), instance('001', 'g3')];
    ...
    expect(getLegalActions(state).some((item) => item.attackerMonster === 0)).toBe(false);
  });

  it('keeps Dodge illegal against an undodgeable attack while allowing compatible Blocks', () => {
    let state = createGame('miracle', 'normal', 'speed', 42);
    state.players[0].hand = [instance('017')];
    state.players[0].guts = [instance('001', 'g1'), instance('002', 'g2'), instance('003', 'g3')];
    state.players[1].hand = [instance('108', 'dodge'), instance('106', 'block')];
    state.players[1].guts = [];
    const action = getLegalActions(state).find((item) => item.label === 'Holy Ray' && item.target?.monster === 2)!;
    state = reduceGame(state, { type: 'play-action', actionId: action.id });
  ...
  it('records ordered defense cards for chained battle cut-ins and replays', () => {
    ...
    const action = getLegalActions(state).find((item) => item.label === 'Stab' && item.target?.monster === 0)!;
    state = reduceGame(state, { type: 'play-action', actionId: action.id });
    state = reduceGame(state, { type: 'play-defense', instanceId: 'endure-071', monster: 0 });
    expect(state.pendingAttack?.attackCards.map((card) => card.cardId)).toEqual(['005']);
    expect(state.pendingAttack?.defenseCards.map((card) => card.cardId)).toEqual(['071']);
    expect(state.pendingAttack?.workingDamage).toBe(2);
  });

  it('enforces one Breeder card per turn', () => {
    let state = createGame('miracle', 'normal', 'speed', 42);
    state.players[0].hand = [instance('112', 'help1'), instance('112', 'help2')];
    state.players[0].guts = [instance('001', 'g1')];
    const action = getLegalActions(state).find((item) => item.label === 'Help')!;
    state = reduceGame(state, { type: 'play-action', actionId: action.id });
    state = reduceGame(state, { type: 'pass-defense' });
    expect(state.players[0].breederCardPlayed).toBe(true);
    expect(getLegalActions(state).some((item) => item.label === 'Help')).toBe(false);
  });
});
...
describe('AI safety and completion', () => {
  it('selects deterministic legal commands from an observation', () => {
    ...
    const request = { observation: observeGame(state, 0), actions: getLegalActions(state), defenses: getLegalDefenses(state), difficulty: 'normal' as const, entropy: 99 };
    expect(chooseAiCommand(request)).toEqual(chooseAiCommand(request));
  });

  for (const player of ['miracle', 'speed', 'powerful'] as const) for (const rival of ['miracle', 'speed', 'powerful'] as const) {
    it(`${player} vs ${rival} completes without illegal moves or deadlock`, () => {
      const result = autoMatch(player, rival, 120 + STARTER_DECKS.findIndex((deck) => deck.id === player) * 10 + STARTER_DECKS.findIndex((deck) => deck.id === rival));
